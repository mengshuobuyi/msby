...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (tableView == _searchTableView) {
        ...
        PostSearchWord *model = _searchHistory[indexPath.row];
        [_searchHistory removeObject:model];
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar {
    ...
    PostSearchWord *modelSearch;
    for (PostSearchWord *word in _searchHistory) {
        if ([word.searchItem isEqualToString:searchword]) {
            modelSearch = word;
        }
    }
    if (modelSearch) {
        [_searchHistory removeObject:modelSearch];
    }
...
-(QWPostModel *)convertModel:(PostSearchVo *)model {
    ...
    newModel.flagAnon = model.flagAnon;
    return newModel;
}
//存搜索历史
...
-(void)operateRpt {
    OpeartionStaticR *modelR = [OpeartionStaticR new];
    modelR.cKey = @"post_search";
    modelR.objRemark = currentSearchWords;
    modelR.token = QWGLOBALMANAGER.configure.userToken;
    modelR.deviceType = 2;
    [OpeartionStatic operateRpt:modelR successs:^{
...
- (void)hotBtnClick:(UIButton *)btn{
    ...
    PostSearchWord *modelSearch;
    for (PostSearchWord *word in _searchHistory) {
        if ([word.searchItem isEqualToString:btn.titleLabel.text]) {
            modelSearch = word;
        }
    }
    if (modelSearch) {
