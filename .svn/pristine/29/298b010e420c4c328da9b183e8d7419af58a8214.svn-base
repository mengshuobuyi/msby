...
- (UIImage *)imageByScalingProportionallyToMinimumSize:(CGSize)targetSize
{
    ...
    if(newImage == nil) DDLogVerbose(@"%s,could not scale image",__func__);
...
- (CGFloat)compressionQualityWithMaxSize:(CGFloat)maxSize {
    ...
    CGFloat imgSize = imageData.length / 1024.0f;//KB
    DDLogVerbose(@"原始文件大小%f",imgSize);
    DDLogVerbose(@"原始压缩比例%f",maxSize / imgSize);
...
-(UIImage *)boxblurImageWithBlur:(CGFloat)blur
{
    ...
    if (error) {
        DDLogVerbose(@"error from convolution %ld", error);
    }
