...
-(void)setFullImagesDeselected:(NSString *)deselectedImage
...
-(void)displayRating:(float)rating {
	...
	lastRating = rating;
    if([self.viewDelegate respondsToSelector:@selector(ratingChanged:)]) {
        [viewDelegate ratingChanged:rating];
    }
    if([self.viewDelegate respondsToSelector:@selector(ratingTouchedWithTag:)]) {
        [viewDelegate ratingTouchedWithTag:self.tag];
    }
}
...
-(void) touchesMoved: (NSSet *)touches withEvent: (UIEvent *)event
{
	...
	float newRating = MIN(5, (float) (pt.x / width) + 1) ;
	if (newRating < 1.0)
		return;
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
	[self touchesMoved:touches withEvent:event];
    if ([self.viewDelegate respondsToSelector:@selector(ratingChangeEnded:)]) {
        [self.viewDelegate ratingChangeEnded:lastRating];
    }
    if([self.viewDelegate respondsToSelector:@selector(ratingTouchedWithTag:)]) {
        [viewDelegate ratingTouchedWithTag:self.tag];
    }
