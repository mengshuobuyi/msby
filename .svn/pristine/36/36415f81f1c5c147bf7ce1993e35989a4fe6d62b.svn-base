    ...
    [queue inDatabase:^(FMDatabase *adb) {
        ...
        [adb makeFunctionNamed:@"StringStartsWithH" maximumArguments:1 withBlock:^(sqlite3_context *context, int aargc, sqlite3_value **aargv) {
            if (sqlite3_value_type(aargv[0]) == SQLITE_TEXT) {
                ...
                DDLogVerbose(@"Unknown formart for StringStartsWithH (%d) %s:%d", sqlite3_value_type(aargv[0]), __FUNCTION__, __LINE__);
        ...
        while ([ars next]) {
            rowCount++;
            DDLogVerbose(@"Does %@ start with 'h'?", [rs stringForColumnIndex:0]);
        }
