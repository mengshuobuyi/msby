    ...
    [QWGLOBALMANAGER readLocationWhetherReLocation:NO block:^(MapInfoModel *mapInfoModel) {
        ...
        [Search homeScanSearchInBranch:modelR success:^(MicroMallSearchProArrayVo *model) {
            ...
            if(model.products.count > 0){
                MicroMallSearchProVo *productVO = model.products[0];
                if(StrIsEmpty(productVO.branchProId)){
                    //店内没有这个商品，则展示TableHeaderView
                    _mainTableView.tableHeaderView = _branchNoDataView;
                }else{
