...
@property (nonatomic, strong) SwitchPharmactAnimationView *switchView;
...
- (void)pushBranchChooseWithBack:(BOOL)show
{
    ...
    viewController.showBack = show;
    [self.currentTabbar.selectedViewController pushViewController:viewController animated:YES];
}
...
+ (NSArray *)tabbarOneItems
{
    ...
    bar1.titleNormalColor = RGBHex(qwColor7);
    ...
    bar2.titleNormalColor = RGBHex(qwColor7);
    ...
    bar3.titleNormalColor = RGBHex(qwColor7);
    ...
    bar4.titleNormalColor = RGBHex(qwColor7);
...
- (BOOL)tabBarController:(UITabBarController *)tabBarController shouldSelectViewController:(UIViewController *)viewController
{
    ...
    if(self.currentTabbar == _tabbarOne) {
        ...
        if(itemRow == 3) {
            ...
            [QWGLOBALMANAGER checkEventId:@"首页_分类tab" withLable:@"首页_分类tab" withParams:setting];
        }else if (itemRow == 0) {
...
- (void)tabBarController:(UITabBarController *)tabBarController didSelectViewController:(UIViewController *)viewController
{
    ...
    if(tabBarController.selectedIndex == 2 && [app isMainTab]) {
        [QWGLOBALMANAGER postNotif:NotiReadNewLocation data:nil object:nil];
    }
}
...
- (void)getNotifType:(Enum_Notification_Type)type data:(id)data target:(id)obj
{
    if(type == NotifForcedTransferToContentScreen) {
    ...
    }else if(NotiReadNewLocation == type) {
        ...
        if(self.showBranchView.superview)
            return;
    ...
    if(type == NotifCityHasChanged){
        ...
        if(!_switchView) {
            _switchView = [[NSBundle mainBundle] loadNibNamed:@"SwitchPharmactAnimationView" owner:self options:nil][0];
        }
        [_switchView showInView:[[UIApplication sharedApplication] keyWindow] withbranchName:((MapInfoModel *)data).branchName];
    }else if (NotiDismissSwitchAnimationLoading == type) {
...
- (void)resetLocation
{
    ...
    [QWGLOBALMANAGER resetLocationInformation:^(MapInfoModel *mapInfoModel) {
        ...
        if(lastMapInfo && [newCityName isEqualToString:lastMapInfo.city]) {
            //仅更新本地地理位置
            [QWGLOBALMANAGER updateMapInfoModelLocation:self.mapInfoModel];
            if(self.currentTabbar == self.tabbarTwo && self.mapInfoModel.status == 3) {
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(alertView.tag == 1008) {
        if(buttonIndex == 1) {
            [self pushBranchChooseWithBack:YES];
...
- (void)pushIntochangeBranchAction
{
    ...
    VC.currentMapInfo = [QWGLOBALMANAGER getMapInfoModel];
    VC.pageType = Enum_ComFrome_Homepage;
    VC.hidesBottomBarWhenPushed = YES;
    [self.currentTabbar.selectedViewController pushViewController:VC animated:YES];
}
