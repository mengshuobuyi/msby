...
//    
    ...
    if (_vo) {//修改地址的情况
        ...
        self.model.city = self.vo.cityName;
        self.model.formattedAddress = self.vo.address;
        self.model.location = [[CLLocation alloc]initWithLatitude:self.vo.lat.floatValue longitude:self.vo.lng.floatValue];
    }else {//添加地址的情况
        self.model = [QWGLOBALMANAGER getMapInfoModel];
        if (!self.model) {//没有定位数据的情况
            self.model = [MapInfoModel new];
            self.model.city = @"苏州";
