...
- (BOOL)open {
    ...
    if(err != SQLITE_OK) {
        DDLogVerbose(@"error opening!: %d", err);
        return NO;
    }
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(_db, nil)) !=0) {
                    DDLogVerbose(@"Closing leaked statement");
            ...
            DDLogVerbose(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (int)busyRetryTimeout {
    ...
    DebugLog(@"FMDB: busyRetryTimeout no longer works, please use maxBusyRetryTimeInterval");
    return -1;
}

- (void)setBusyRetryTimeout:(int)i {
    DebugLog(@"%s:%d", __FUNCTION__, __LINE__);
    DebugLog(@"FMDB: setBusyRetryTimeout does nothing, please use setMaxBusyRetryTimeInterval:");
}
...
- (void)warnInUse {
    DDLogVerbose(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!_db) {
        ...
        DDLogVerbose(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                ...
                DebugLog(@"DB Path: %@", _databasePath);
            }
    ...
    if (dictionaryArgs) {
        ...
        for (NSString *dictionaryKey in [dictionaryArgs allKeys]) {
            ...
            if (_traceExecution) {
                DebugLog(@"%@ = %@", parameterName, [dictionaryArgs objectForKey:dictionaryKey]);
            }
            ...
            if (namedIdx > 0) {
                ...
                DebugLog(@"Could not find index for %@", dictionaryKey);
            }
        }
    }
    else {
    ...
    if (idx != queryCount) {
        DDLogVerbose(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
    ...
    if (!pStmt) {
        ...
        if (SQLITE_OK != rc) {
            if (_logsErrors) {
                DDLogVerbose(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                DDLogVerbose(@"DB Query: %@", sql);
                DDLogVerbose(@"DB Path: %@", _databasePath);
            }
    ...
    if (idx != queryCount) {
        DDLogVerbose(@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql);
        sqlite3_finalize(pStmt);
        _isExecutingStatement = NO;
        return NO;
    }
    ...
    if (SQLITE_DONE == rc) {
        ...
        if (_logsErrors) {
            DDLogVerbose(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(_db));
            DDLogVerbose(@"DB Query: %@", sql);
        }
    }
        ...
        // uh oh.
        if (_logsErrors) {
            DDLogVerbose(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(_db));
            DDLogVerbose(@"DB Query: %@", sql);
        }
    }
    else {
        // wtf?
        if (_logsErrors) {
            DDLogVerbose(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(_db));
            DDLogVerbose(@"DB Query: %@", sql);
        }
    }
    ...
    if (closeErrorCode != SQLITE_OK) {
        if (_logsErrors) {
            DDLogVerbose(@"Unknown error finalizing or resetting statement (%d: %s)", closeErrorCode, sqlite3_errmsg(_db));
            DDLogVerbose(@"DB Query: %@", sql);
        }
    }
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
    ...
    if (errmsg && [self logsErrors]) {
        DDLogVerbose(@"Error inserting batch: %s", errmsg);
        sqlite3_free(errmsg);
    }
