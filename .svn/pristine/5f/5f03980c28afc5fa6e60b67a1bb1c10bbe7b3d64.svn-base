...
- (void)popVCAction:(id)sender
{
    ...
    if(self.mapInfoModel)
        [QWUserDefault setModel:self.mapInfoModel key:kLastLocationSuccessAddressModel];
}
...
- (void)setupSearchBarUI{
    ...
    UIView *searchView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, APP_W, 43.0f)];
    searchView.backgroundColor = RGBHex(qwColor21);
    
    UIView *searchBar = [[UIView alloc]initWithFrame:CGRectMake(8, 7, APP_W - 16, 29)];
    [searchView addSubview:searchBar];
    searchBar.backgroundColor = RGBHex(qwColor4);
    searchBar.layer.masksToBounds = YES;
    searchBar.layer.cornerRadius = 14.5f;
    
    UIView *view = [[UIView alloc]initWithFrame:CGRectMake((APP_W - 115)/2.0f, 0, 105, 25)];
    UIImageView *image = [[UIImageView alloc]initWithFrame:CGRectMake(0, 5.5f, 20, 20)];
    image.image = [UIImage imageNamed:@"icon_adr_search"];
    [view addSubview:image];
    
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(30, 0, 100, 29)];
...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section{
    ...
    UILabel *label = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, APP_W, 31.0f)];
    label.backgroundColor = RGBHex(qwColor25);
    label.textColor = RGBHex(qwColor24);
