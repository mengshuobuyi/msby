...
@implementation OrderBaseModelR

@end

@implementation OrderBaseCommentModelR
...
- (NSString *)description {
    NSDictionary *dictModel = [self dictionaryModel];
    NSXMLElement *rootXml = [NSXMLElement elementWithName:@"xml"];
    for(NSString *key in [dictModel allKeys]) {
        [rootXml addChild:[NSXMLElement elementWithName:key stringValue:dictModel[key]]];
    }
    
    return [rootXml XMLString];
}
