...
- (void)setNaviBar
{
    ...
    lblTitle.text = @"免费问药";
    lblTitle.font = fontSystem(kFontSize15);
...
- (void)setTextViewPlaceholder:(BOOL)didSet
{
    if (didSet) {
        if (self.btnQuizOne.selected) {
            self.lblPlaceholder.text = kWarning10;
        } else {
            self.lblPlaceholder.text = kWarning11;
        }
        
        NSLog(@"the palce holder frame is %@",NSStringFromCGRect(self.lblPlaceholder.frame));
...
- (IBAction)pushToChatList:(id)sender {
    ...
    NSString *strConsultContent = [self.tvConsult.text stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (self.showPlaceholder) {
        [self showError:kWarning13];
        return;
    } else {
        if (strConsultContent.length == 0) {
            [SVProgressHUD showErrorWithStatus:kWarning13 duration:DURATION_SHORT];
            return;
    ...
    if (!self.btnQuizOne.selected && !self.btnQuizTwo.selected) {
        [SVProgressHUD showErrorWithStatus:kWarning14 duration:DURATION_SHORT];
        return;
    }
