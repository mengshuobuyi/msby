...
@property (weak, nonatomic) IBOutlet NSLayoutConstraint *actionBtnHeight;

+ (BOOL)shoudShowActionBtnWith:(MsgBoxNoticeItemModel *)model;
...
- (void)setCell:(id)obj
{
    ...
    if ([MsgBoxNoticeCell shoudShowActionBtnWith:model]) {
        self.actionBtnHeight.constant = 42.0;
        self.actionBtn.hidden = NO;
        self.midSeperator.hidden = NO;
    } else {
        self.actionBtnHeight.constant = 0;
        self.actionBtn.hidden = YES;
        self.midSeperator.hidden = YES;
    }
    if (model.objType.integerValue == MsgBoxNoticeTypeCoupon || model.objType.integerValue == MsgBoxNoticeOldTypeCoupon) {
...
//        }
        self.contentImageView.image = [UIImage imageNamed:@"bg_quan_news"];
    } else {
        [self.contentImageView setImageWithURL:[NSURL URLWithString:model.imgUrl] placeholderImage:[UIImage imageNamed:@"bg_default_news"]];
    }
...
+ (BOOL)shoudShowActionBtnWith:(MsgBoxNoticeItemModel *)model
{
    return [@[@(MsgBoxNoticeTypeCoupon),
              @(MsgBoxNoticeTypeReport),
              @(MsgBoxNoticeTypeRebindPhone),
              @(MsgBoxNoticeTypeCreditStore),
              @(MsgBoxNoticeTypeGoodsCredit),
              @(MsgBoxNoticeTypeOPOutLink),
              @(MsgBoxNoticeTypeOPShopSales),
              @(MsgBoxNoticeTypeOPNews),
              @(MsgBoxNoticeTypeOPTopic),
              @(MsgBoxNoticeTypeOPCoupon),
              @(MsgBoxNoticeTypeOPGoodsSales),
              @(MsgBoxNoticeTypeOPNewsTopic),
              @(MsgBoxNoticeTypeOPPostDetail),
              @(MsgBoxNoticeTypeCreditChanged),
              @(MsgBoxNoticeOldTypeCoupon),
              @(MsgBoxNoticeOldTypeGoodsSales),
              @(MsgBoxNoticeOldTypeMemberPromote),
              @(MsgBoxNoticeOldTypeOPOutLink),
              @(MsgBoxNoticeOldTypeOPNews),
              @(MsgBoxNoticeOldTypeOPTopic),
              @(MsgBoxNoticeOldTypeOPShopSales),
              @(MsgBoxNoticeOldTypeOPCoupon),
              @(MsgBoxNoticeOldTypeOPGoodsSales),
              @(MsgBoxNoticeOldTypeSlowDiease),
              @(MsgBoxNoticeOldTypeOPTopicDetail),
              @(MsgBoxNoticeOldTypeOPPostDetail),
              @(MsgBoxNoticeOldTypeDietDetail)] containsObject:@(model.objType.integerValue)];
}
