...
- (void)setUpData:(id)data
{
    ...
    //头像
    [self.headerIcon setImageWithURL:[NSURL URLWithString:model.headImageUrl] placeholderImage:[UIImage imageNamed:@"img_pharmacy"]];
    ...
    if (model.userType == 3 || model.userType == 4) {
        ...
        NSString *logoName;
        NSString *store;
        if (model.userType == 3) { //药师 标识显示药师logo及所属商家
            store = model.groupName;
            logoName = @"pharmacist";
        }else if (model.userType == 4){ //营养师 标识显示营养师logo及“营养师”
            store = @"营养师";
            logoName = @"ic_expert";
        }
        NSDictionary* style1 = @{@"thumb":[UIImage imageNamed:logoName]};
        NSAttributedString * tempText = [[NSString stringWithFormat:@"%@  <thumb> </thumb>  %@",name,store] attributedStringWithStyleBook:style1];
        NSMutableAttributedString *nameAttribute = [[NSMutableAttributedString alloc] initWithAttributedString:tempText];
        NSRange nameRange = [[nameAttribute string] rangeOfString:[NSString stringWithFormat:@"%@",store]];
        ...
        [nameAttribute addAttribute:NSForegroundColorAttributeName
                              value:RGBHex(qwColor8)
                              range:nameRange];
        self.name.attributedText = nameAttribute;
    }else{
