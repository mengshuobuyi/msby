...
- (void)syncNewCircleMsgList:(NSArray *)arrList
{
    ...
    for (CircleChatPointModel *model in arrList) {
        ...
        if (indexFound != NSNotFound) {
            ...
            if (modelMessage.respondDate.doubleValue < model.respondDate.doubleValue) {
                        ...
                        failure:(void(^)(HttpException * e))failure
{
    ...
    [Circle setAllCircleMsgListReadWithParams:modelR success:^(BaseModel *model) {
        if (success) {
            ...
            [QWGLOBALMANAGER noticeUnreadByMsgBoxTypes:@[@(MsgBoxListMsgTypeExpertPTP),@(MsgBoxListMsgTypeCircle)] readFlag:YES];
