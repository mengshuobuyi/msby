...
{
    contentView = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width, 38)];
    ...
    label.textColor = RGBHex(qwColor2);
    [contentView addSubview:label];
    UIImageView *detailView = [[UIImageView alloc] initWithFrame:CGRectMake(self.view.frame.size.width-15, 13, 12, 12)];
...
- (void)setupSearchBar
{
    self.searchBar = [[UISearchBar alloc] initWithFrame:CGRectMake(0, 0, self.view.frame.size.width-55,44)];
...
- (void)queryMedicineWithKeyword:(NSString *)keyword
{
    ...
    queryProductByKeywordModelR.province=mapInfoModel==nil?@"江苏省":mapInfoModel.province;
    queryProductByKeywordModelR.city=mapInfoModel==nil?@"苏州市":mapInfoModel.city;
...
- (UITableViewCell *)tableView:(UITableView *)atableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(cell == nil){
        ...
        view_bg.backgroundColor = RGBHex(qwColor10);
    ...
    [cell setCell:productByKeywordModel];
    return cell;
}
...
- (void)tableView:(UITableView *)atableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(self.selectBlock)
    {
        ...
        modelTwo.objRemark=productByKeywordModel.proName;
        modelTwo.objId=productByKeywordModel.proId;
        modelTwo.cKey=@"e_family_add_product";
        [QWGLOBALMANAGER qwChannel:modelTwo];
        
        
        GetProductUsageModelR *getProductUsageModelR = [GetProductUsageModelR new];
        getProductUsageModelR.proId = productByKeywordModel.proId;
        
        [Drug getProductUsageWithParam:[getProductUsageModelR dictionaryModel] Success:^(id DFUserModel) {
            NSDictionary *dataRow = @{@"productName":productByKeywordModel.proName,
                                      @"productId":productByKeywordModel.proId
                                      };
            
            NSMutableDictionary *source = [NSMutableDictionary dictionaryWithDictionary:dataRow];
            [source addEntriesFromDictionary:[DFUserModel dictionaryModel]];
            if(source[@"dayPerCount"]) {
                source[@"drugTime"] = source[@"dayPerCount"];
                [source removeObjectForKey:@"dayPerCount"];
            }
            if(productByKeywordModel.imgUrl) {
                source[@"imgUrl"] = productByKeywordModel.imgUrl;
            }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(buttonIndex == 1)
    {
        ...
        if(self.selectBlock) {
            NSMutableDictionary *dict = [NSMutableDictionary dictionaryWithObjectsAndKeys:drugName,@"productName", nil];//@{@"productName":drugName} ;
