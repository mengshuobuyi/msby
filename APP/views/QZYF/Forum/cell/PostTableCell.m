...
- (void)setCell:(id)obj
{
    if ([obj isKindOfClass:[QWPostModel class]]) {
        ...
        if (postModel.flagAnon) {
            self.userImageView.image = ForumDefaultImage;
...
-(void)setCell:(id)obj WithHighlightKeyWords:(NSString *)keywords {
    [self setCell:obj];
     QWPostModel* postModel = obj;
    NSString *title = postModel.postTitle;
    NSRange range =  [title rangeOfString:keywords];
    NSDictionary *attr = [NSDictionary dictionaryWithObjectsAndKeys:RGBHex(qwColor2),NSForegroundColorAttributeName, nil];
    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc]initWithString:title];
    [attStr addAttributes:attr range:range];
    self.postTitleLabel.attributedText = attStr;
}
