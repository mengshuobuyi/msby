 ...
 *  此控件不会因为上拉/下拉产生任何回调
 */
- (void)addStaticImageHeader
{
    // 1.创建新的header
    if (!self.header) {
        MJRefreshHeaderView *header = [MJRefreshHeaderView header];
        header.useStaticImage = YES;
        [self addSubview:header];
        self.header = header;
    }
}
...
- (void)setFooterNoDataText:(NSString *)footerNoDataText{
    self.footer.noDataText=footerNoDataText;
}

- (NSString *)footerNoDataText{
    return self.footer.noDataText;
}
...
- (UIImageView *)headerRefreshImage
{
    return self.header.imgBg;
}

- (UIImageView *)refreshPatternImageView
{
    return self.header.refreshPatternImageView;
}
