...
- (void)reloadData {
    ...
    self.emotionPageControl.numberOfPages = 5;
...
- (void)setup {
    ...
    if (!_emotionCollectionView) {
        ...
        CGFloat collectViewHeight = CGRectGetHeight(self.bounds) - kXHEmotionPageControlHeight - kXHEmotionSectionBarHeight;
        
        XHEmotionCollectionViewFlowLayout *flowLayout = [[XHEmotionCollectionViewFlowLayout alloc] init];
        flowLayout.itemSize = CGSizeMake(APP_W/7, (collectViewHeight - 20)/3);
        //        CGFloat padding = (APP_W - (7*kXHEmotionImageViewSize))/7;
        CGFloat padding = 0;
        //        CGSize referenceSize = CGSizeMake(padding/2, CGRectGetHeight(self.bounds) - kXHEmotionPageControlHeight - kXHEmotionSectionBarHeight);
        flowLayout.minimumLineSpacing = padding;
        //        flowLayout.headerReferenceSize = referenceSize;
        //        flowLayout.footerReferenceSize = referenceSize;
        
        UICollectionView *emotionCollectionView = [[UICollectionView alloc] initWithFrame:CGRectMake(0, 0, APP_W, collectViewHeight) collectionViewLayout:flowLayout];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    XHEmotionManager *emotionManager = [self.dataSource emotionManagerForColumn:self.selectedIndex];
    XHEmotionManager *subEmotionManager = emotionManager.emotions[indexPath.row];
    XHEmotion *emotion = [[XHEmotion alloc] init];
    emotion.emotionConverPhoto = [UIImage imageNamed:subEmotionManager.imageName];
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath {
    if ([self.delegate respondsToSelector:@selector(didSelecteEmotion:atIndexPath:)]) {
        ...
        XHEmotion *emotion = [[XHEmotion alloc] init];
        XHEmotionManager *subEmotionManager = [emotionManager.emotions objectAtIndex:indexPath.row];
        emotion.emotionConverPhoto = [UIImage imageNamed:subEmotionManager.imageName];
        emotion.emotionPath = subEmotionManager.emotionName;
        [self.delegate didSelecteEmotion:emotion atIndexPath:indexPath];
    }
}
