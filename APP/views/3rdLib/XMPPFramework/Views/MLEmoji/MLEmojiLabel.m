...
CGFloat kEmojiOriginYOffsetRatioWithLineHeight = 0.10; //表情绘制的y坐标矫正值，和字体高度的比例，越大越往下
...
NSString * const kURLActions[] = {@"url->",@"phoneNumber->",@"email->",@"at->",@"poundSign->",@"drugGuide->",@"drugDetail->"};
    ...
    for (id line in lines) {
        ...
        for (id glyphRun in (__bridge NSArray *)CTLineGetGlyphRuns((__bridge CTLineRef)line)) {
            ...
            if (imageName) {
                ...
                if([imageName isEqualToString:@"ic_expert"] || [imageName isEqualToString:@"pharmacist"]) {
                    runBounds.size.width = 11;
                    runBounds.size.height = 15;
                    kEmojiOriginYOffsetRatioWithLineHeight = 0.001;
...
- (void)addLinkTags:(NSArray *)tagList
{
    ...
    for(TagWithMessage *tag in tagList)
    {
...
+ (CGSize)needSizeWithText:(NSString *)emojiText WithConstrainSize:(CGSize)size FontSize:(CGFloat)floatSize
{
    MLEmojiLabel *displayTextView = [[MLEmojiLabel alloc] init];
    displayTextView.numberOfLines = 0;
    displayTextView.customEmojiRegex = @"\\[[a-zA-Z0-9\\u4e00-\\u9fa5]+\\]";
    ...
    displayTextView.disableThreeCommon = YES;
    [displayTextView setEmojiText:emojiText];
    displayTextView.font = [UIFont systemFontOfSize:floatSize];
    displayTextView.lineBreakMode = NSLineBreakByWordWrapping;
    displayTextView.isNeedAtAndPoundSign = YES;
    displayTextView.minimumLineHeight = floatSize+10;
    CGSize retSzie = [displayTextView sizeThatFits:size];
    retSzie.height = MAX(floatSize+10, retSzie.height);
    return retSzie;
}
...
didSelectLinkWithTextCheckingResult:(NSTextCheckingResult *)result;
{
    ...
    }else if (result.resultType == NSTextCheckingTypeLink)
    {
        if(self.emojiDelegate && [self.emojiDelegate respondsToSelector:@selector(mlEmojiLabel:didSelectLink:withType:)])
        {
            ...
            for(TagWithMessage *msg in self.tagList)
            {
                NSString *subString = [self.emojiText substringWithRange:NSMakeRange([msg.start integerValue], [msg.length integerValue])];
                if([string isEqualToString:subString])
                {
                    if([msg.tagType integerValue] == 1)
                    {
                        //表示药品
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:msg.tagId withType:MLEmojiLabelLinkTypeMedicineDetail];
                    }else if ([msg.tagType isEqualToString:@"outerLink"]) {
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:msg.tagId withType:MLEmojiLabelLinkTypeOuterLink];
                    }else if ([msg.tagType isEqualToString:@"postDetail"]) {
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:msg.tagId withType:MLEmojiLabelLinkTypePostDetail];
                    }
                    else{
                        [self.emojiDelegate mlEmojiLabel:self didSelectLink:msg.tagId withType:MLEmojiLabelLinkTypeDrugGuide];
                    }
                    break;
                }
            }
            if(self.tagList.count == 0){
                [self.emojiDelegate mlEmojiLabel:self didSelectLink:@"" withType:MLEmojiLabelLinkTypeQuickSearch];
            }
        }
    }
}
...
- (void)initPlist
{
    ...
    //    NSString *documentDir = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    //    NSString *doc = [NSString stringWithFormat:@"%@/expression.plist",documentDir];
    //    DDLogVerbose(@"%@,length:%ld",doc,testArray.count);
    //    if ([testArray writeToFile:doc atomically:YES]) {
    //        DDLogVerbose(@"归档expression.plist成功");
    //    }
    //    doc = [NSString stringWithFormat:@"%@/expressionImage.plist",documentDir];
