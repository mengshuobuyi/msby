...
- (void)layoutSubviews
{
    ...
    if (_isBuildUI) {
        ...
        //滚动到选中的视图
            DebugLog(@"----------------_userSelectedChannelID %d",_userSelectedChannelID);
...
//    }
    ...
    if (self.slideSwitchViewDelegate && [self.slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchView:didselectTab:)]) {
        DDLogVerbose(@"%d",_userSelectedChannelID - 100);
...
- (void)buildUI
{
    ...
    if (self.slideSwitchViewDelegate && [self.slideSwitchViewDelegate respondsToSelector:@selector(slideSwitchView:didselectTab:)]) {
        DDLogVerbose(@"select delegate is %d",_userSelectedChannelID - 100);
//        [self.slideSwitchViewDelegate slideSwitchView:self didselectTab:_userSelectedChannelID - 100];
    }
...
//        btnS.selected = YES;
    ...
    UIView *line= [[UIView alloc]initWithFrame:CGRectMake(0, _topScrollView.frame.size.height - 0.5, APP_W, 0.5)];
...
- (void)createNameButtons
{
    ...
    if([_viewArray count] > 4) {
        if(self.isCollectAdjust){
         buttonWidth = MAX(buttonWidth, 40);
        }else{
...
//            button.selected = YES;
    ...
    //设置顶部滚动视图的内容总尺寸
    DDLogVerbose(@"the button width is %f, margin width is %f, arr count is %d", buttonWidth,kWidthOfButtonMargin,_viewArray.count);
    if (_viewArray.count > 4) {
        ...
        _topScrollView.contentSize = CGSizeMake(APP_W, kHeightOfTopScrollView);
    }
...
- (void)adjustScrollViewContentX:(UIButton *)sender
{
    ...
    if (sender.frame.origin.x - _topScrollView.contentOffset.x + kWidthOfButtonMargin + sender.frame.size.width >= self.bounds.size.width - (kWidthOfButtonMargin+sender.bounds.size.width)) {
        ...
        if (self.isNeedAdjust) {
            ...
            if (((sender.frame.origin.x + sender.frame.size.width + 80) >= self.bounds.size.width)&&(tagSelect < [_viewArray count] - 1)) {
