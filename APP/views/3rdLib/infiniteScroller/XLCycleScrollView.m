...
- (void)setupCustomPageControl
{
    ...
    CGRect rect = self.bounds;
    rect.origin.y = rect.size.height - 25;
    rect.origin.x = rect.size.width - 200 - 10;
    rect.size.width = 200;
    rect.size.height = 30;
    
    _customPageControl = [[SMPageControl alloc] initWithFrame:rect];
    _customPageControl.alignment = SMPageControlAlignmentRight;
    _customPageControl.indicatorMargin = 7.0f;
    _customPageControl.userInteractionEnabled = NO;
    [self addSubview:_customPageControl];
    [self.customPageControl setPageIndicatorImage:[UIImage imageNamed:@"icon_switch_homepage_rest"]];
    [self.customPageControl setCurrentPageIndicatorImage:[UIImage imageNamed:@"icon_switch_homepage_selected"]];

}

- (void)updatePager
{
//    _customPageControl.currentPage = _curPage;
}
...
- (void)reloadData
{
    ...
    _customPageControl.numberOfPages = _totalPages;
    [self loadData];
}
...
//{
//    CGPoint offset = CGPointMake(_scrollView.frame.size.width * _customPageControl.page, 0);
