...
{
    [super UIGlobal];
    [self.tableMain setBackgroundColor:RGBHex(qwColor21)];
}
...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    QWGLOBALMANAGER.currentShopConsultBranchID = self.branchId;
    
}

- (void)setupPhoneView
{
    phoneLabel = [[UILabel alloc]initWithFrame:CGRectMake(15, 10, 260, 20)];
    phoneLabel.text = @"您也可以直接拨打药房电话咨询";
    phoneLabel.font  = [UIFont systemFontOfSize:12.0];
    phoneLabel.textColor = RGBHex(qwColor6);
    phoneLabel.backgroundColor = [UIColor clearColor];
    phoneBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    
    phoneView = [[UIImageView alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.view.bounds), CGRectGetMinY(self.view.bounds), APP_W, 40)];
    ...
    phoneBgView = [[UIView alloc]initWithFrame:CGRectMake(CGRectGetMinX(self.view.bounds), CGRectGetMinY(self.view.bounds), APP_W, 40)];
    
    phoneBgView.backgroundColor = [UIColor clearColor];
    phoneBtn.frame = CGRectMake(APP_W-56, 5, 30, 30);
    [phoneBtn setImage:[UIImage imageNamed:@"icon_phone_consult"] forState:UIControlStateNormal];
...
//    if ([self.title isEqualToString:@""] || self.title == nil || [self.title isKindOfClass: [NSNull class]]) {
...
//    }
    ...
    //初始化营销活动控件,这里可以优化,仅在第一次选择时初始化  @lazy
...
- (void)viewWillDisappear:(BOOL)animated
{
    ...
    QWGLOBALMANAGER.currentShopConsultBranchID = nil;
}


//进店咨询,如果之前界面已经有该家药店详情则直接回退至上层界面
...
- (MessageModel *)buildSpecialOffersShowOnceMessage:(id)drugModel
{
    ...
    MessageModel *message = [[MessageModel alloc] initWithSpecialOffersShowOnce:self.coupnDetailModel.title content:self.coupnDetailModel.desc activityUrl:self.coupnDetailModel.url activityId:self.coupnDetailModel.id sender:self.messageSender timestamp:lastTimeStamp UUID:[XMPPStream generateUUID]];
...
- (void)messageCenterInit
{
    if (msgCenter == nil){
        ...
        msgCenter= [[PTPMessageCenter alloc]initWithID:self.branchId type:IMTypePTPClient];
...
//            self.tableMain.hidden = NO;
        ...
        if (gotType == IMListAll) {
            if(self.sendConsultType == Enum_SendConsult_Drug && self.product) {
            ...
            // 远端拉取全量自动设置已读
            [QWGLOBALMANAGER noticeUnreadLocalWithType:MsgBoxListMsgTypeShopConsult sessionID:self.sessionID isRead:YES];
...
//            }];
    ...
    [self messageToPharMsg:messageModel send:MessageDeliveryState_Delivering];
}
...
//将MessageModel转换成PharMsgModel,并保存至DB
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if(model.messageDeliveryType == MessageTypeSending) {
        ...
        if (model.messageDeliveryType == MessageTypeSending) {
            ...
            [cell.headImageView setImageWithURL:[NSURL URLWithString:model.avatorUrl] placeholderImage:[UIImage imageNamed:@"ic_yaofang_pepole"] options:SDWebImageHighPriority];
        }
        if (displayTimestamp) {
...
//        [SJAvatarBrowser showImage:(UIImageView *)imageView];
    ...
    {
        //发送药品链接
        [QWGLOBALMANAGER statisticsEventId:@"咨询店长_发送药品链接按键" withLable:nil withParams:nil];
        MessageModel *model = [userInfo objectForKey:KMESSAGEKEY];
    ...
    {
        [QWGLOBALMANAGER statisticsEventId:@"咨询店长_药品链接" withLable:nil withParams:nil];
        //V4.0 商品详情
        MessageModel *model=[userInfo objectForKey:KMESSAGEKEY];

        //直接跳转商品详情，永远只跳转关联药房下的商品详情
...
//        OnlineCouponVoModel *newModel = [OnlineCouponVoModel new];
//        
//        newModel.couponId = mm.otherID;
//                newModel.groupId = mm.title;
//        newModel.couponValue = mm.richBody;
//        newModel.chronic = mm.isStyle;
//        newModel.begin = mm.arrList.firstObject;
//        newModel.end = mm.arrList.lastObject;
...
- (void)sendMarketActivityTextMessageWithHTTP:(MessageModel *)model
{
    ...
    [self messageToPharMsg:model send:MessageDeliveryState_Delivering];
}
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    showPhone = NO;
    NSIndexPath *ip = [self.tableMain indexPathForRowAtPoint:CGPointMake(0, scrollView.contentOffset.y-15.0)];
    MessageModel *message =  [msgCenter getMessageByIndex:ip.row];
    if (  message.messageMediaType == MessageMediaTypePhone) {
        phone = ip.row;
        showPhone = YES;
    }
...
-(void)showOrHideHeaderView
{
    ...
    if (!StrIsEmpty(phoneNumber))
    {
        NSIndexPath *tableViewVisbale = [[self.tableMain indexPathsForVisibleRows] firstObject];
        //           DDLogVerbose(@"showOrHideHeaderView--phoneNumber---%@",phoneNumber);
        for (NSInteger i = tableViewVisbale.row; i <msgCenter.count ; i++) {
            MessageModel *message =  [msgCenter getMessageByIndex:i];
            if (  message.messageMediaType == XHBubbleMessageMediaTypePhone) {
                phoneView.hidden = YES;
                phoneBtn.hidden = YES;
                //                DDLogVerbose(@"showOrHideHeaderView--phoneNumber--i-%d",i);
