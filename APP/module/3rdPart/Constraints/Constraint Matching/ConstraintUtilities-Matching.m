...
// This ignores any priority and any constant, looking only at y (R) mx
- (BOOL) isEqualToLayoutConstraintIgoreConstant: (NSLayoutConstraint *) constraint
{
    // I'm still wavering on these two checks
    if (![self.class isEqual:[NSLayoutConstraint class]]) return NO;
    if (![self.class isEqual:constraint.class]) return NO;
...
- (NSLayoutConstraint *) constraintMatchingConstraintWithIgnoreConstant: (NSLayoutConstraint *) aConstraint
{
    NSArray *views = [@[self] arrayByAddingObjectsFromArray:self.superviews];
    for (VIEW_CLASS *view in views)
        for (NSLayoutConstraint *constraint in view.constraints)
            if ([constraint isEqualToLayoutConstraintIgoreConstant:aConstraint])
                return constraint;
